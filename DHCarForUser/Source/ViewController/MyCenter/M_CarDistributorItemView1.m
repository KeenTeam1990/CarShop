...
@interface M_CarDistributorItemView1 ()
...
@implementation M_CarDistributorItemView1
DEF_FACTORY_FRAME(M_CarDistributorItemView1);
...
-(void)layoutSubviews
{
    ...
    tempframe.origin.x = self.bounds.size.width-tempframe.size.width-KLeftOffset;
...
{
    self.myNameLabel = [self getLabel:CGRectMake(KLeftOffset, frame.origin.y+(frame.size.height-30)/2+5, frame.size.width-KLeftOffset*2, 30)];
...
-(void)initRow2View:(CGRect)frame
{
    ...
    self.myAddrLabel = [self getLabel:CGRectMake(self.myAddrIconView.frame.origin.x+tempIcon.size.width+5, frame.origin.y, frame.size.width-100-KLeftOffset*2-tempIcon.size.width,frame.size.height)];
    [self.myAddrLabel setNumberOfLines:3];
    self.myAddrLabel.lineBreakMode= NSLineBreakByCharWrapping;
}
...
-(void)initRow3View:(CGRect)frame
{
    ...
    self.myTellIconView = [[UIImageView alloc]initWithFrame:CGRectMake(KLeftOffset, frame.origin.y+3, tempIcon.size.width, tempIcon.size.height)];
    self.myTellIconView.image = tempIcon;
    [self addSubview:self.myTellIconView];
    
    self.myDistanceLabel = [self getLabel:CGRectMake(frame.size.width-60-KLeftOffset, frame.origin.y , 20, 30)];
    [self.myDistanceLabel setFont:[UIFont systemFontOfSize:12]];
    [self.myDistanceLabel setTextAlignment:UITextAlignmentRight];
    
    self.myTellLabel = [self getLabel:CGRectMake(self.myTellIconView.frame.origin.x+tempIcon.size.width+5, frame.origin.y, frame.size.width-80-KLeftOffset*2-tempIcon.size.width, 20)];
}
...
-(void)updateData:(M_DealerItemModel*)data
{
    ...
    if (data!=nil) {
        ...
        if (self.showSelete) {
            ...
            self.mySeleteIconView.hidden = !data.selete;
            UIImage* tempIcon = [UIImage imageNamed:@"icon_addr.png"];
...
//    }
    ...
    self.myTellLabel.hidden = !self.showCellPhone;
    self.myTellIconView.hidden = !self.showCellPhone;
