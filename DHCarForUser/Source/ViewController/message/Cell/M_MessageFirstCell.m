...
AS_MODEL_STRONG(M_MyMessageItemModel , myItemModel);//实际数据模型
...
-(id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        self.custom_HeadView.layer.masksToBounds = YES;
        self.custom_HeadView.layer.cornerRadius = 30;
...
//        [self.contentView addSubview:self.myLineView];
        ...
        self.time_MessageLable=[[UILabel alloc]initWithFrame:CGRectMake(self.bounds.size.width-160, CGRectGetMinY(self.custom_NameLable.frame), 150, 30)];
        ...
        self.myLine2View = [[UIImageView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(self.custom_HeadView.frame), self.bounds.size.width, 10)];
...
-(void)layoutSubviews
{
    ...
    tempframe.origin.x = self.bounds.size.width-160;
...
-(void)updataItemModel:(M_MyMessageItemModel *)model
{
    if (model!=nil) {
        ...
        self.custom_HeadView.image = [UIImage imageNamed:@"icon_head.png"];
        
        if (model.myFromModel!=nil) {
            if ([model.myFromModel.user_id isEqualToString:[DLAppData sharedInstance].myUserKey]) {
                
            }else{
                if ([model.myFromModel.user_photo notEmpty]) {
                    [self.custom_HeadView setImageWithURL:[NSURL URLWithString:model.myFromModel.user_photo] placeholderImage:[UIImage imageNamed:@"icon_head.png"]];
                }
                if ([model.myFromModel.user_name notEmpty]) {
                    self.custom_NameLable.text = model.myFromModel.user_name;
                }
            }
        }
        
        if (model.myToModel!=nil) {
            if ([model.myToModel.user_id isEqualToString:[DLAppData sharedInstance].myUserKey]) {
                
            }else{
                if ([model.myToModel.user_photo notEmpty]) {
                    [self.custom_HeadView setImageWithURL:[NSURL URLWithString:model.myToModel.user_photo] placeholderImage:[UIImage imageNamed:@"icon_head.png"]];
                }
                if ([model.myToModel.user_name notEmpty]) {
                    self.custom_NameLable.text = model.myToModel.user_name;
                }
            }
        }
        
        if ([model.myContent notEmpty]) {
            self.message_TextLable.text = model.myContent;
        }
        
        if ([model.myTimer notEmpty]) {
           
            self.time_MessageLable.text = [Unity getTimeFromTimerLong:model.myTimer];
        }
    }
}
