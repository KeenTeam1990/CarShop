...
//AS_MODEL_STRONG(M_FieldOrButtonView, myCommentPhoneFBView);
...
@implementation M_ReserveFooterView

DEF_FACTORY_FRAME(M_ReserveFooterView);
...
-(instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [self.myPhoneFBView updateSetting:YES withShowBtn:NO withShowDrowBtn:YES withLabel:@"*手机号码:" withPlaceholder:@"请输入您的手机号码"];
...
//        self.myCommentPhoneFBView = [M_FieldOrButtonView allocInstanceFrame:CGRectMake(0, tempHeight+KLineHeight*3, frame.size.width, KLineHeight)];
//        [self.myCommentPhoneFBView updateSetting:YES withShowBtn:NO withShowDrowBtn:NO withLabel:@"推荐账号:" withPlaceholder:@"输入推荐人账号"];
//        [self addSubview:self.myCommentPhoneFBView];
        ...
        [self.myMemoFBView getTextView:CGRectMake(KLeftOffset+KLabelWidth, 10, frame.size.width-KLabelWidth-KLeftOffset*2, frame.size.height-20) withImage:@"灰色输入框.png" withPlaceholder:nil];
        ...
        //[self.myPhoneFBView updateContent:@"13910291094"];
//        [self.myCommentPhoneFBView updateContent:@"13910291094"];
    }
    return self;
}
...
-(void)initData
{
    __weak M_ReserveFooterView* tempSelf = self;
...
//    self.myCommentPhoneFBView.block = ^(NSInteger tag,id data){
//        // 0 seleteBtn
//        // 1 btn
//        switch (tag) {
//            case 1:
...
//            case 2:
//                {
//                    tempSelf.currFieldRect = tempSelf.myCommentPhoneFBView.frame;
//                }
//                break;
//            case 3:
//                {
...
//    NSString* tempCode = [self.myCommentPhoneFBView getText];
    
    if ([tempName empty]) {
        [SVProgressHUD showErrorWithStatus:@"请输入姓名"];
        return;
    }
    if (!(tempName.length>=2&&tempName.length<=10)) {
        [SVProgressHUD showErrorWithStatus:@"输入的姓名字符长度请保持在2位到10位,请您重新输入"];
        return;
    }
